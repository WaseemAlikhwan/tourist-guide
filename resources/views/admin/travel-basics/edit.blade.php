...
@section('title', 'تعديل أساسيات السفر | لوحة التحكم')
...
@section('content')
    ...
    <div class="page-header">
        <div>
            <h1>تعديل أساسيات السفر: {{ $travelBasic->title }} 🧳</h1>
            <p class="subtitle">تحديث تفاصيل أساسيات السفر</p>
        </div>
    ...
    <div class="card card-custom">
        <div class="card-body">
            <form action="{{ route('admin.travel-basics.update', $travelBasic) }}" method="POST">
                @csrf
                @method('PUT')
                    ...
                    <div class="col-md-8">
                        <div class="mb-4">
                            ...
                            <input type="text"
                                   ...
                                   id="title_ar"
                                   name="title_ar"
                                   value="{{ old('title_ar', $travelBasic->title_ar) }}"
                                   required>
                            @error('title_ar')
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text"
                                   ...
                                   id="title_en"
                                   name="title_en"
                                   value="{{ old('title_en', $travelBasic->title_en) }}">
                            @error('title_en')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text"
                                   ...
                                   id="icon"
                                   name="icon"
                                   value="{{ old('icon', $travelBasic->icon) }}"
                            ...
                            @if($travelBasic->icon)
                                <div class="mt-2">
                                    ...
                                    <i class="{{ $travelBasic->icon }}" style="font-size: 1.5rem; color: var(--primary);"></i>
                                </div>
                            @endif
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <div class="btn-group w-100" role="group">
                                @php
                                    $contentType = old('content_type', ($travelBasic->items_ar && count($travelBasic->items_ar) > 0) ? 'list' : 'text');
                                @endphp
                                <input type="radio" class="btn-check" name="content_type" id="content_type_list" value="list" {{ $contentType == 'list' ? 'checked' : '' }} onchange="toggleContentType()">
                                <label class="btn btn-outline-primary" for="content_type_list">
                                    <i class="bi bi-list-ul"></i> قائمة عناصر
                                </label>

                                <input type="radio" class="btn-check" name="content_type" id="content_type_text" value="text" {{ $contentType == 'text' ? 'checked' : '' }} onchange="toggleContentType()">
                        ...
                        <!-- محتوى القائمة -->
                        <div id="listContent" style="display: {{ $contentType == 'list' ? 'block' : 'none' }};">
                            <div class="mb-4">
                        ...
                        <!-- محتوى النص -->
                        <div id="textContent" style="display: {{ $contentType == 'text' ? 'block' : 'none' }};">
                            <div class="mb-4">
                                ...
                                <textarea class="form-control @error('content_ar') is-invalid @enderror"
                                          id="content_ar"
                                          name="content_ar"
                                          rows="8">{{ old('content_ar', $travelBasic->content_ar) }}</textarea>
                                @error('content_ar')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="mb-4">
                                ...
                                <textarea class="form-control @error('content_en') is-invalid @enderror"
                                          id="content_en"
                                          name="content_en"
                                          rows="8">{{ old('content_en', $travelBasic->content_en) }}</textarea>
                                @error('content_en')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="card-body">
                                ...
                                <div class="mb-4">
                                    ...
                                    <input type="number"
                                           ...
                                           id="order"
                                           name="order"
                                           value="{{ old('order', $travelBasic->order) }}"
                                           min="0">
                                    @error('order')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                ...
                                <div class="form-check form-switch mb-3">
                                    <input class="form-check-input" type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $travelBasic->is_active) ? 'checked' : '' }}>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // تحميل العناصر المحفوظة مسبقاً
        @php
            $existingItemsAr = old('items_ar', $travelBasic->items_ar ?? []);
            $existingItemsEn = old('items_en', $travelBasic->items_en ?? []);
            $maxItems = max(count($existingItemsAr), count($existingItemsEn));
        @endphp
        @if($maxItems > 0)
            @for($i = 0; $i < $maxItems; $i++)
                addItem(
                    {!! json_encode($existingItemsAr[$i] ?? '') !!},
                    {!! json_encode($existingItemsEn[$i] ?? '') !!}
                );
            @endfor
        @else
            addItem();
        ...
        @php
            $existingSectionsAr = old('custom_sections_ar', $travelBasic->custom_sections_ar ?? []);
            $existingSectionsEn = old('custom_sections_en', $travelBasic->custom_sections_en ?? []);
